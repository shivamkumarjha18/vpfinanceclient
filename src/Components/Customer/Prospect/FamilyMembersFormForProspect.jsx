...
const FamilyMembersFormForProspect = ({ prospectId, prospectData, onProspectCreated }) => {
  const dispatch = useDispatch();
  const [familyMembers, setFamilyMembers] = useState([]);

  useEffect(() => {
    // If editing, load existing members from suspectData
    if (prospectData && prospectData.familyMembers) {
      setFamilyMembers(prospectData.familyMembers);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("prospect id from family ", prospectId) 
    ...
    const result = await dispatch(addFamilyMember({ prospectId: idToUse, membersArray: familyMembers }));
    if (result) {
      setFamilyMembers([]);
      toast.info("Family Members Added Successfully...");
      const returnedId = result?.payload;
      console.log("Prospect ID in family details:", returnedId);
      if (onProspectCreated && returnedId) onProspectCreated(returnedId);
    }
  };
