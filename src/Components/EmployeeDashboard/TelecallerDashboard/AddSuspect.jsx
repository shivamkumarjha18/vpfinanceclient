...
//       setFormData({
//         NextCallDate: suspectData.NextCallDate || '',
...
//     try {
//       const response = await axios.get(`http://localhost:8080/api/suspect/${suspectId}/call-history`);
//       if (response.data.success) {
//         setSuspectData(response.data);
//       }
//     } catch (error) {
...
//     e.preventDefault();
//     try {
//       const response = await axios.post(`http://localhost:8080/api/suspect/${suspectId}/call-task`, {
//         taskDate: formData.NextCallDate,
//         taskTime: formData.Time,
//         taskRemarks: formData.callRemarks,
//         taskStatus: formData.callStatus,
//       });

//       if (response.data.success) {
//         alert('Call task added successfully!');
//         fetchCallHistory(); // Refresh call history after adding a task
//       }
//     } catch (error) {
//       console.error('Error adding call task:', error);
//       alert('Failed to add call task. Please try again.');
//     }
//   };
...
//   textAlign: "center" 
// }}>
//   <h3 style={{ marginBottom: "10px", color: "#333" }}>working procedure</h3>
//   <button 
//     style={{ 
//       padding: "8px 16px", 
...
const AddSuspect = ({ isEdit, suspectData, onSuspectCreated }) => {
  ...
  // Rest of your useEffect hooks here
  ...
  const fetchCallHistory = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/suspect/${suspectData._id}/call-history`);
      if (response.data.success) {
        setSuspectData(response.data);
      }
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`http://localhost:8080/api/suspect/${suspectData._id}/call-task`, {
      ...
      if (response.data.success) {
        ...
        fetchCallHistory(); // Refresh call history after adding a task
        setFormData2({ NextCallDate: '', callStatus: '', Time: '', callRemarks: '' }); // Reset form
      }
    } catch (error) {
      console.error('Error adding call task:', error);
      toast.error('Failed to add call task. Please try again.');
    }
  };

  // handleMobileWhatsappChange, fetchOccupations, fetchOccupationTypes functions remain unchanged
